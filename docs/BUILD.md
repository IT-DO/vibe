...
**Releases → `android-latest` → файл `photo-na-pamyat.apk`**
...
adb install -r photo-na-pamyat.apk
```
...
## Путь 3. Приложение для iPad — заморожено

> **Ветка iOS не развивается.** Проект настроен, нативные модули подключены,
> компиляция под симулятор проверена в CI — но работа сосредоточена на
> Android. Инструкция ниже оставлена рабочей на случай разморозки; учтите,
> что с момента заморозки её никто не проверял на живой сборке.
>
> Сборка под iOS убрана из автоматических прогонов. Запуск вручную:
> **Actions → «Сборка» → Run workflow → галка «Собрать также под iOS»**.
>
> Почему так: на Android нет ежегодной платы и сертификатов, APK ставится
> файлом, а закрепление приложения — настоящий системный киоск-режим, тогда
> как на iOS это «Гид-доступ», который надо включать руками после каждой
> перезагрузки.
...
## Как узнать причину сбоя на устройстве

Приложение записывает падения само — компьютер и `adb` для этого не нужны.

**Админка → «Журнал ошибок»**: там видно число записей, последние три
целиком и две кнопки — «Поделиться» (отправить журнал любым мессенджером) и
«Очистить».

Перехватчик стоит на двух уровнях: нативном (ловит падения Java, даже когда
JavaScript уже не выполняется) и на уровне JavaScript. Именно нативный уровень
здесь решающий: падение из-за отсутствующего разрешения `VIBRATE` произошло в
нативном потоке, и обработчик JS его бы не увидел.

Журнал лежит во внутренней папке приложения, сам никуда не отправляется и
обрезается при росте свыше 128 КБ.

Если всё же нужен полный системный лог (на компьютере с Android SDK,
при включённой отладке по USB на телефоне):
