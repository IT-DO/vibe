...
describe('печать готового снимка из галереи', () => {
  ...
  it('опоздавший отказ печати не пугает следующего гостя', () => {
    // Отказ очереди может прийти уже после возврата к заставке. Показать
    // из-за него экран ошибки означало бы встретить им того, кто только
    // подошёл.
    const {state} = run(
      [{type: 'printFailed', message: 'Принтер не отвечает', now: 1_000}],
      config,
    );
    expect(state.name).toBe('attract');
  });
  ...
  it('галерея не открылась — гость видит ответ, а не пустой экран', () => {
    ...
    const {state, effects} = run(
      [{type: 'pickFailed', message: 'Нет доступа к галерее', now: 1_000}],
      config,
    );
